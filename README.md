...
- Publish-audio contract jobs resolve one canonical shared daily theme per target date, calendar, and locale, then reuse that payload across Morning Prayer, Daily Rosary, Daily Reflection, Auxilium Christianorum, and Marian Antiphons so their sidecars carry the same theme title, Gospel bridge, Gospel citation, fallback status, and source list for the day.
...
- Daily Reflection publishes as its own date-scoped Ora Pro Nobis episode with the shared daily liturgical theme helper, a short pause-rich Ignatian-style reflection, a guided examen, deterministic fallback text when OpenAI is unavailable, and sidecar metadata for the helper/reflection source.
...
- `jobs/publish/daily_liturgical_context.py`: shared daily liturgical context helper that normalizes feast, Gospel, season, saint, tone, imagery, music mood, and inclusive daily theme metadata for generated prayer and reflection experiences
