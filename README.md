...
- Publish contracts support non-spoken `audio_cue` and `pause` blocks. The built-in `sacred_bell` cue is synthesized and cached without TTS, explicit pauses carry a validated duration and purpose, and both controls are omitted from readable prayer text and resume markers.
